...
class Insercao extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('insercao_model', '', TRUE);
        $this->data['menuProdutos'] = 'Insercao';
    }
    ...
    function gerenciar(){
        ...
        $config['base_url'] = base_url().'index.php/insercao/gerenciar/';
        $config['total_rows'] = $this->insercao_model->count('insercao');
        $config['per_page'] = 10;
	    ...
	    $this->data['results'] = $this->insercao_model->get('insercao','idinsercao,notafiscal,datainsercao,codproduto,quantidade','',$config['per_page'],$this->uri->segment(3));
       
	    $this->data['view'] = 'insercao/insercao';
    ...
    function adicionar() {
        ...
        if ($this->form_validation->run('insercao') == false) {
            $this->data['custom_error'] = (validation_errors() ? '<div class="form_error">' . validation_errors() . '</div>' : false);
        } else {
            $data = array(
                'notafiscal' => set_value('notafiscal'),
				'codproduto' => set_value('descricao'),
				'quantidade' => set_value('estoque')
            );

            if ($this->insercao_model->add('insercao', $data) == TRUE) {
                $this->session->set_flashdata('success','Produto adicionado com sucesso!');
                redirect(base_url() . 'index.php/insercao/adicionar/');
            } else {
                $this->data['custom_error'] = '<div class="form_error"><p>An Error Occured.</p></div>';
            }
        }
        $this->data['view'] = 'insercao/adicionarInsercao';
        $this->load->view('tema/topo', $this->data);
